...
我们分别在 Parser 中写了 `parseProg` 和 `parseSayHi` 两个方法，它们分别对应语法中的 `prog` 和 `say_hi` 这两个规则。然后我们根据 `prog` 的语法规则，在 `parseProg` 内部调用了 `parseSayHi`，方法直接的协作方式，完全的参照了语法规则的定义。
...
按照规则的定义，我们在 `parseExpr` 内部，需要首先调用 `parseExpr` 方法。很显然，由于这个方法直接调用了自身、且内部并没有对需要处理的字符串进行任何步进操作，因此会陷入无限循环。

对于一个规则而言，如果它的右边最先出现的又是自身的规则，我们就称该规则是左递归「Left Recursion」的；并且该规则所属的语法，又被称为是左递归的语法。

## 消除左递归「Elimination of Left Recursion」

我们目前手写的语法解析器，是属于自上而下「Top-Down」的类型，因此我们发现，自上而下的语法解析器，是无法处理左递归语法的。
...
我们发现 A 又出现了在其右边规则内容的第一个位置。所以和上面介绍的、我们的自上而下的解析器无法处理直接左递归一样，对应间接左递归，同样无法处理。

先看对应直接左递归 `A ::= Aα | β` 而言，它如何匹配输入的字符串：
...
这样的目的就是先匹配 `βααα...` 中的第一个 `β`，并且满足了我们的重写需求。

接着 `A'` 要做的就是能够匹配重复出现的 `α`，因此我们可以将 `A'` 写成：
...
和我们消除后的语法类似，我们使用方法来表示语法规则中的展开操作。`parseNum` 就是解析数值字面量，就不过多解释了。
...
我们还没有考虑如何消除间接左递归，因为消除直接左递归已经可以应对大部分情况了，我们不妨等遇到间接左递归的时候再考虑如何消除它。
