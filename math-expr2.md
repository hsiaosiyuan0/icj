...
我们来一下在 [解析算术表达式 - 左递归和其消除法](math-expr.md) 末尾的测试用例，对于表达式 `1 + 2 + 3`，我们输出的结果为
...
我们还一起看了这段代码执行的流程示意图。这段代码将操作符都处理成右结合性的原因就在于，`parseExpr1` 方法中，对于右边节点的处理，总是调用 `parseExpr`：
...
非常简单的修改，只有一行 `node.right = this.parseExpo();`，我们将原本 `parseFactor` 换成了 `parseExpo`。
